...
# 梯度下降版线性回归
def LR_GD(n_inputs, n_outputs, X, y, lr=1e-3, batch_size=128, epochs=1e5):
...
if __name__ == "__main__":
    ...
    for i_num in num_traj_arr:
        num_traj = i_num
        print("当前训练的轨迹数：", num_traj)
        
        algo = 'linear'
        model_path = f"data/ROBO/controllers/{algo}/controller/2_models_{num_traj}_s_{seed}_lr_{lr}_e_{int(epochs)}/"
        ...
        # noisy datas
        # systems_path = "output/robo_train_noisy_random_0.5/systems"
        # data_path = 'output/robo_train_noisy_random_0.5/'

        systems_path = "output/robo_train/systems"
        data_path = 'output/robo_train/'
        ...
        for system_name in systems_list:
            ...
            if num_traj == 10:
                ...
                pattern = data_path + system_name + '_' + '[0-4]?' + '.csv'
            elif num_traj == 60:
                pattern = data_path + system_name + '_' + '[0-5]?' + '.csv'
            ...
            # my GD LR
            model = LR_GD(n_inputs, n_outputs, X_train, y_train,lr=lr,epochs=epochs)
