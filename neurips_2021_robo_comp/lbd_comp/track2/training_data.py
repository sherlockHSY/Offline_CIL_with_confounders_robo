...
def generate_training_data(output_dir, num_training_trajs, force_output=False, debug_output=False):
    ...
    for sysid, system in enumerate(tqdm(systems)):
        ...
        mark_noisy = 0
        noisy_traj_num = int(num_training_trajs/2)
        ...
        for repetition, (startpos, targetpos, k) in enumerate(
            ...
            for t in range(S._timesteps - 1):
                ...
                current_obsv = S[:, t]  # state，包含每个机械臂的尖端坐标
                reference_pose = desired_ee_traj[t]  # 目标坐标
                inp = robot_ctrl.get_input(current_obsv, reference_pose)  # inp即 U
                S.step(u=inp)
                # if is_add_noise:

                #     u = np.random.normal() * sigma
                #     noisy_inp = inp + 0.2 * u + 0.5 * u_past
                #     S.step(u=noisy_inp)
                #     u_past = u
                # else:
                #     S.step(u=inp)
        ...
        print("noisy traj num: ", mark_noisy)
